...
export function SensorList({ className }: SensorListProps) {
  const { sensors, reliabilityScores, selectedSensorId, selectSensor } =
    useDashboard();
              ...
              >
                ...
                <div className="mt-3 flex justify-between items-baseline">
                  <div className="text-lg font-semibold">
                    {sensor.value}{" "}
                    <span className="text-xs text-zinc-500">{sensor.unit}</span>
                  </div>
                ...
                {reliability && (
                  <div className="mt-3">
                    <ReliabilityScore score={reliability.score} size="sm" />
                  </div>
                )}
              </div>
            );
